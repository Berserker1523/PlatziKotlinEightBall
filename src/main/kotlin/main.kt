...
fun mostrarRespuestasPorTipo(tipoDeRespuesta : String = "TODOS") {
    ...
    when (tipoDeRespuesta){
        "TODOS" -> imprimirRespuestas(respuestas) //respuestas.keys.forEach { respuesta -> println(respuesta) }
        ...
        RESPUESTA_NEGATIVA -> respuestas.filterValues { values -> values == RESPUESTA_NEGATIVA }
            .also(imprimirRespuestas) //{ respuestasNegativas -> println(respuestasNegativas.keys) }

        RESPUESTA_DUDOSA -> respuestas.filterValues { values -> values == RESPUESTA_DUDOSA }
            .also(imprimirRespuestas) //{ respuestasDudosas -> println(respuestasDudosas.keys) }
    }
}
